...
// Load environment variables locally if not already set (Vercel does this automatically)
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // 4. Upload ID proof to Cloudinary
    let idProofUrl = null;
    try {
      const uploadIdRes = await cloudinary.uploader.upload(id_proof.base64, {
        folder: "champions_cup/id_proofs",
        resource_type: "auto" // handles PDFs or images automatically
      });
      idProofUrl = uploadIdRes.secure_url;
    } catch (uploadErr) {
      console.error("Cloudinary ID Proof upload error:", uploadErr);
      return res.status(500).json({ success: false, message: "Failed to upload ID proof document to Cloudinary storage." });
    }

    // 5. Upload Payment receipt to Cloudinary
    let paymentReceiptUrl = null;
    try {
      const uploadReceiptRes = await cloudinary.uploader.upload(payment_receipt.base64, {
        folder: "champions_cup/receipts",
        resource_type: "auto"
      });
      paymentReceiptUrl = uploadReceiptRes.secure_url;
    } catch (uploadErr) {
      console.error("Cloudinary Receipt upload error:", uploadErr);
      return res.status(500).json({ success: false, message: "Failed to upload payment receipt screenshot to Cloudinary storage." });
    }

    // 6. Insert details into Turso DB
    await dbClient.execute({
